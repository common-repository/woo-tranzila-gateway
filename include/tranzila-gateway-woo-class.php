...
class WC_Tranzila_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->version_of_iframe          = $this->get_option('version_of_iframe');
		$this->type_of_currency           = $this->get_option('type_of_currency');
		$this->language_of_iframe         = $this->get_option('language_of_iframe');
		$this->order_status_after_payment = $this->get_option('order_status_after_payment');
		$this->aсtivate_installment       = $this->get_option('aсtivate_installment');
		$this->installment_maxpay         = $this->get_option('installment_maxpay');
	...
	function init_form_fields(){
		$this->form_fields = array(
			'tranzila_settings_steps' => array(
				...
				'type'        => 'title',
				'description' => __( '<p>1. Get terminal name from the Tranzila company and provide it in <strong>Terminal Name</strong> field below.</p><p>2. Copy and insert next URL for <strong>success URL</strong> and <strong>fail URL</strong> fields in Tranzila Sales page settings:</p><code>'.home_url('/wc-api/tranzila_payment_callback').'</code>' ),
			),
			'enabled'     => array(
			...
			'version_of_iframe' => array(
				'title'         => __('Version of iframe', 'wctranzillapc'),
				'type'          => 'select',
				'description'   =>  __('Choose version of iframe', 'wctranzillapc'),
				'required'      => true,
				'options'       => array(
			...
			'type_of_currency' => array(
				'title'       => __('Type of currency', 'wctranzillapc'),
				'type'        => 'text',
				'description' =>  __('Leave 1 for ILS', 'wctranzillapc'),
				'default'     =>  '1',
			),
			'language_of_iframe' => array(
				'title'         => __('Iframe language', 'wctranzillapc'),
				'type'          => 'select',
				'description'   =>  __('Choose language of iframe', 'wctranzillapc'),
				'required'      => true,
				'options'       => $this->tgwc_get_iframe_languages(),
			),
			'order_status_after_payment' => array(
				'title'         => __('Order status after payment', 'wctranzillapc'),
				'type'          => 'select',
				'description'   =>  __('Choose order status after successful payment', 'wctranzillapc'),
				'required'      => true,
				'options'       => wc_get_order_statuses(),
				'default'       => 'processing'
			),
			'aсtivate_installment'     => array(
				'title'   => __('Aсtivate installment', 'wctranzillapc'),
				'type'    => 'checkbox',
				'label'   => __('Activates the opportunity of an installment for your customers', 'wctranzillapc'),
				'default' => 'no'
			),
			'installment_maxpay' => array(
				'title'       => __('Maxpay', 'wctranzillapc'),
				'type'        => 'number',
				'min'         => '1',
				...
				'description' => __('Maximum amount of installments. F.e. 3.', 'wctranzillapc'),
				'default'     => 1
			),
		);
	}
	...
	function tgwc_receipt_page( $order ) {
		...
		$iframe_version = ( isset($this->version_of_iframe) && !empty($this->version_of_iframe) ) ? $this->version_of_iframe : 'new_version';

		$iframe_version_php_path = $this->tgwc_get_iframe_version_path($iframe_version);

		$lang = ( isset($this->language_of_iframe) && !empty($this->language_of_iframe) ) ? $this->language_of_iframe : 'il';
		...
		if ( empty($type_of_currency) || !(int)$type_of_currency > 0 ) {
			$type_of_currency = '1'; //default ILS
		}

		//installment
		$installment = '';
		if ( $this->aсtivate_installment == 'yes' ) {
			$maxpay = $this->installment_maxpay;
			if ( (int)$maxpay > 0 ) {
				//cred_type=8 it is required to send the value 8 for installments
				$installment .= '&maxpay='.$maxpay.'&cred_type=8';
			}
		}

		$iframe = '<iframe src="https://direct.tranzila.com/'.$this->terminal_name.'/'.$iframe_version_php_path.'?lang='.$lang.'&sum='.$order_total.'&cy=10&currency='.$type_of_currency.$installment.'&email='.$order_email.'&phone='.$order_phone.'&address='.$order_address.'&contact='.$order_contact.'&pdesc=WooCommerce Data: '.$woo_data.'" frameborder="0" width="400" height="650"></iframe>';
				...
				if ( $received_code == $real_code[0] ) {
					...
					if ( $real_total == $recieved_total ) {
						...
						if (!isset($_POST['Response'])) {
							...
							die ($result . "\n");
						} elseif ( sanitize_text_field($_POST['Response']) !== '000') {
							...
							$order_status = ( isset($this->order_status_after_payment) && !empty($this->order_status_after_payment) ) ? $this->order_status_after_payment : 'wc-processing';  //default woo order status after successful payment
	...
	function tgwc_get_iframe_languages() {
		$langs = array(
			'il' => 'Hebrew',
			'us' => 'English',
			'ru' => 'Russian',
	...
	function tgwc_get_iframe_version_path($version) {
		$path = array(
			'new_version' => 'iframenew.php',
			'old_version' => 'iframe.php',
		);
